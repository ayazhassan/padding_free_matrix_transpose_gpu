...
__global__ void transposeParallelWrite(float *odata, float *idata, int width, int height, int nreps)
{
  __shared__ float tile[TILE_DIM][TILE_DIM];
  ...
  int y_bar = threadIdx.x;
  int x_bar = (threadIdx.x + threadIdx.y) % TILE_DIM;
  for (int r=0; r < nreps; r++) {
    for (int i=0; i<TILE_DIM; i+=BLOCK_ROWS) {
      tile[y_bar+i][x_bar] = idata[index_in+i*width];
    }
	...
	y_bar = threadIdx.y;
    for (int i=0; i<TILE_DIM; i+=BLOCK_ROWS) {
      odata[index_out+i*height] = tile[x_bar][y_bar+i];
    }
  }
}

__global__ void transposeSelectiveCopying(float *odata, float *idata, int width, int height, int nreps)
{
	__shared__ float tile[TILE_DIM][TILE_DIM];
	int xIndex = blockIdx.x * TILE_DIM + threadIdx.x;
	int yIndex = blockIdx.y * TILE_DIM + threadIdx.y;
	...
	int x_diagonal = (threadIdx.x + threadIdx.y) % TILE_DIM;
	int y_diagonal = threadIdx.x;

	if(y_diagonal > x_diagonal){
		float temp1 = tile[y_diagonal][x_diagonal];
		float temp2 = tile[x_diagonal][y_diagonal];
		tile[x_diagonal][y_diagonal] = temp1;
		tile[y_diagonal][x_diagonal] = temp2;
	}
...
__global__ void transposeDiagonalCOE501(float *odata, float *idata, int width, int height, int nreps)
{
  __shared__ float tile[TILE_DIM][TILE_DIM];
...
int main( int argc, char** argv) 
{
  ...
  for (int k = 0; k<11; k++) {
    ...
    case 8:
      kernel = &transposeParallelWrite;        kernelName = "Parallel-Write    "; break;
    case 9:
      kernel = &transposeNoBankConflicts;       kernelName = "optimized         "; break;
    case 10:
      kernel = &transposeSelectiveCopying;			kernelName = "Selective-Copying "; break;
    }      
    ...
    shrLog("\n");
    shrLog("transpose-Outer-%s, Throughput = %.4f GB/s, Time = %.5f s, Size = %u fp32 elements, NumDevsUsed = %u, Workgroup = %u\n", 
           kernelName,
           outerBandwidth, 
           outerTime/NUM_REPS, 
           (size_x * size_y), 1, TILE_DIM * BLOCK_ROWS); 
    shrLog("transpose-Inner-%s, Throughput = %.4f GB/s, Time = %.5f s, Size = %u fp32 elements, NumDevsUsed = %u, Workgroup = %u\n", 
           kernelName,
           innerBandwidth, 
           innerTime/NUM_REPS, 
           (size_x * size_y), 1, TILE_DIM * BLOCK_ROWS);
